...
[Challenge files](./ezheap2.7z)

A heap exploitation menu challenge.

<!--more-->

Challenge: [ezheap2.7z](ezheap2.7z)
...
Calls `malloc` to allocate memory for a given index, with controllable size.

### edit

Calls `read` to write content to a specified index's memory. Contains an off-by-one vulnerability.

![edit_off_by_one](edit_off_by_one.png)

### delete

Calls `free` to release memory at a specified index.

### gift

Entering anything other than 1 leaks the PIE base address.

## Strategy

1. Allocate several physically contiguous memory blocks. Combine the off-by-one to create chunk overlapping and attack `tcache_entry->next` for arbitrary address write.
2. Corrupt `_IO_2_1_stdout_` to leak the libc base address.
3. Hijack `__free_hook` to ROP.
